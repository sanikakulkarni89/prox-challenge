import { getRecentDiagnoses, getUserPreferences, type WeldDiagnosis } from './weldMemory';

export const BASE_SYSTEM_PROMPT = `You are OmniPro, the AI expert assistant for the Vulcan OmniPro 220 multiprocess welding system (Harbor Freight item #57812).
...
function buildMemoryBlock(
  diagnoses: WeldDiagnosis[],
  prefs: Awaited<ReturnType<typeof getUserPreferences>>,
): string {
  const lines: string[] = ['', '---', '', '## OPERATOR MEMORY'];

  // OPERATOR HISTORY — one line per diagnosis
  lines.push('', 'OPERATOR HISTORY:');
  for (const d of diagnoses) {
    const p = d.parameters;
    const process = str(p.process) || str(prefs?.last_process) || 'unknown process';
    const material = str(p.material) || str(prefs?.last_material) || 'unknown material';
    const voltageVal = num(p.voltage) ?? prefs?.last_voltage;
    const voltage = voltageVal != null ? `${voltageVal}A` : 'unknown amperage';
    const outcome = d.outcome ?? 'no outcome recorded';
    lines.push(`- ${formatDate(d.created_at)} ran ${process} on ${material} at ${voltage} — result: ${outcome}.`);
  }

  // PREVIOUS DEFECTS — grouped by type with count and last corrective action
  const defectMap = new Map<string, { count: number; lastAction: string }>();
  for (const d of diagnoses) {
    const type = d.defect_type;
    const action =
      str(d.parameters.corrective_action) ||
      str(d.parameters.correction) ||
      str(d.parameters.action) ||
      'none recorded';
    const existing = defectMap.get(type);
    if (!existing) {
      defectMap.set(type, { count: 1, lastAction: action });
    } else {
      // diagnoses are newest-first so the first entry is already the last action
      defectMap.set(type, { count: existing.count + 1, lastAction: existing.lastAction });
    }
  }

  lines.push('', 'PREVIOUS DEFECTS:');
  if (defectMap.size === 0) {
    lines.push('- none on record');
  } else {
    for (const [type, { count, lastAction }] of Array.from(defectMap.entries())) {
      lines.push(`- ${type} x${count} — last corrective action: ${lastAction}`);
    }
  }
  ...
  if (prefs) {
    ...
    const parts: string[] = [];
    if (prefs.last_process) parts.push(`process: ${prefs.last_process}`);
    if (prefs.last_material) parts.push(`material: ${prefs.last_material}`);
    if (prefs.last_voltage != null) parts.push(`voltage: ${prefs.last_voltage}V`);
    if (prefs.last_wire_speed != null) parts.push(`wire speed: ${prefs.last_wire_speed} IPM`);
    if (parts.length > 0) lines.push(`- Last settings — ${parts.join(', ')}`);
...
export async function buildSystemPrompt(userId: string): Promise<string> {
  const [diagnoses, prefs] = await Promise.all([
    getRecentDiagnoses(userId, 5),
  ...
  if (!hasHistory && !hasPrefs) {
    return BASE_SYSTEM_PROMPT;
  }

  const memoryBlock = buildMemoryBlock(diagnoses, prefs);

  // Inject memory block just before the artifact style guide (last major section)
  const insertionMarker = '## Artifact Generation Style Guide';
  const markerIndex = BASE_SYSTEM_PROMPT.indexOf(insertionMarker);
  ...
  return (
    BASE_SYSTEM_PROMPT.slice(0, markerIndex) +
    memoryBlock +
    BASE_SYSTEM_PROMPT.slice(markerIndex)
  );
}
